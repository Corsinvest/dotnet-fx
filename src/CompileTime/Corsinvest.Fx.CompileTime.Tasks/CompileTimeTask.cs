...
public class CompileTimeTask : Microsoft.Build.Utilities.Task, IDisposable
{
    ...
    private async Task<bool> ExecuteInternalAsync(CancellationToken cancellationToken)
    {
        ...
        {
            // Use simple logging to avoid MSBuild ArgumentNullException with projectFile
            Log.LogError($"CompileTime execution failed: {ex.GetType().Name}: {ex.Message}");
        ...
        finally
        {
            try
            {
                ...
                // Save diagnostics as code for CompileTimeDiagnosticAnalyzer
                await SaveDiagnosticsAsCodeAsync(cancellationToken);
    ...
    private async System.Threading.Tasks.Task SaveDiagnosticsAsCodeAsync(CancellationToken cancellationToken)
    {
        try
        {
            if (_diagnostics.Count == 0) { return; }

            var diagnosticsFile = Path.Combine(OutputPath, "CompileTimeDiagnostics.g.cs");
            ...
            var sb = new StringBuilder();
            sb.AppendLine("// <auto-generated />");
            sb.AppendLine("// Generated by CompileTime MSBuild Task - Diagnostic Information");
            sb.AppendLine("#nullable enable");
            sb.AppendLine();
            sb.AppendLine("namespace CompileTime");
            sb.AppendLine("{");
            sb.AppendLine("    /// <summary>");
            sb.AppendLine("    /// Contains diagnostic information from CompileTime execution.");
            sb.AppendLine("    /// This class is used to provide diagnostic information to the analyzer.");
            sb.AppendLine("    /// </summary>");
            sb.AppendLine("    [global::System.Runtime.CompilerServices.CompilerGeneratedAttribute()]");

            // Generate diagnostic attributes
            foreach (var diagnostic in _diagnostics)
            {
                var escapedMessageArgs = string.Join(", ", diagnostic.MessageArgs?.Select(arg => arg != null ? $"@\"{arg.ToString()?.Replace("\"", "\"\"")}\"" : "null") ?? []);
                sb.AppendLine($$"""    [global::Corsinvest.Fx.CompileTime.CompileTimeDiagnosticAttribute("{{diagnostic.Id}}", @"{{diagnostic.FilePath}}", {{diagnostic.StartPosition}}, {{diagnostic.Length}}, new object[] { {{escapedMessageArgs}} })]""");
            }

            sb.AppendLine("    internal static class CompileTimeDiagnosticsContainer");
            sb.AppendLine("    {");
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("}");
            ...
            await File.WriteAllTextAsync(diagnosticsFile, sb.ToString(), cancellationToken);
            Log.LogMessage(MessageImportance.Low, "Saved {0} diagnostics as code to {1}", _diagnostics.Count, diagnosticsFile);
        }
        catch (Exception ex)
        {
            Log.LogWarning("Failed to save diagnostics as code: {0}", ex.Message);
        }
    }
