...
real nested types. A plain `switch` works as well as `Match()`.

`Ok<T>` and `Fail<E>`'s cases each close over a different one of `ResultOf<T, E>`'s own type
parameters - the shape that only the `IUnion<T1..T8>` interface can express, since an attribute
argument cannot reference the decorated type's own type parameters (see
...
variable - `ok` is an `Ok<User>`, `fail` is a `Fail<ValidationError>` - and its own `Value` /
`ErrorValue` holds the payload:

```csharp
string Describe(ResultOf<User, ValidationError> result) => result switch
{
    ResultOf<User, ValidationError>.Ok(var ok) => $"✅ {ok.Value.Name}",
...
{
    ValidationResult.Ok(var ok) => $"✅ {ok.Value.Name}",
    ValidationResult.Fail(var fail) => $"❌ {fail.ErrorValue}"
};
```
...
```csharp
// Auto-generated from IUnion<Ok<T>, Fail<E>> (concise names)
